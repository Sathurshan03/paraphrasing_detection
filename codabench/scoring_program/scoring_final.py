...
with open(os.path.join(prediction_dir, 'predictions.txt'), 'r') as prediction_file:
    ...
    test_labels = pd.read_json(os.path.join(reference_dir, 'test_label.json'))

    ## convert to to float list
    test_label_list = test_labels['label'].tolist()




print('Checking Accuracy')
accuracy = (1.0 - mean_squared_error(test_label_list, prediction)) * 100.0
print('Scores:')
